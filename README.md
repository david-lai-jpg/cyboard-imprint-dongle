...
|---------|--------|-----------------|----------|
| 0 | Left PMW3610 | `trackball_central_listener` | Scroll (1/32x, Y-inverted, accel curve) |
| 1 | Right PMW3610 | `trackball_peripheral_listener` | Mouse cursor (33/20x, linear accel) |
...
Using the `imprint_function_row_full_bottom_row` physical layout (82 keys).
...
- **Dual trackballs** -- left trackball for scrolling (1/32x sensitivity, Y-inverted, Bezier acceleration curve), right trackball for cursor (33/20x sensitivity, linear acceleration)
- **Acceleration curves** -- Bezier curve-based scroll acceleration via [`zmk-acceleration-curves`](https://github.com/david-lai-jpg/zmk-acceleration-curves) fork with `default-curve` DT property (flash and forget, no manual NVS shell config needed)
- **ZMK Studio** -- enabled on the dongle build for runtime keymap editing over USB
...
|----------|-------|--------|-------------|
| `imprint_dongle` | `nice_nano_v2` | `imprint_dongle dongle_display` | Dongle central (USB + OLED + ZMK Studio) |
...
└── west.yml                    # West manifest (ZMK + Cyboard + accel curves modules)
